...
2. Get the provider address of the service. You can enter just the domain or the full base URL — both work. For example, if Naga AI's API url is <https://api.naga.ac/v1/chat/completions>, you can enter either "api.naga.ac" or "https://api.naga.ac/v1" in the Yantra Launcher Settings by using the `settings` command. Yantra adds `/v1/chat/completions` automatically when it is missing, and assumes `https://` when you leave out the scheme.
